...
def _within(now: datetime.datetime, start: datetime.time, end: datetime.time) -> bool:
    """
    Compare en (hour, minute) pour éviter les problèmes naïf/aware.
    """
    t = now.time()  # tzinfo peut être None -> on compare en tuples (h, m)
    t_hm = (t.hour, t.minute)
    s_hm = (start.hour, start.minute)
    e_hm = (end.hour, end.minute)
    return s_hm <= t_hm <= e_hm
